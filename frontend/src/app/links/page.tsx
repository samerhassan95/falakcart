  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[44px]  text-[#191C1E] tracking-tight">My Referral Links</h1>
          <p className="text-[#505F76] mt-1 text-[16px]">Create, manage, and track your affiliate links across all campaigns.</p>
        </div>
          <button
...
              style={{ background: 'linear-gradient(135deg, #2A14B4 0%, #4338CA 100%)' }}          >
        ...
        <div className="bg-white rounded-2xl border border-gray-100  overflow-hidden">
          <div className="p-6 flex items-center justify-between border-b border-gray-50">
            <h3 className="text-lg font-bold text-[#191C1E]">Active Campaigns</h3>
            <div className="relative">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#505F76]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
              <input
                type="text"
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {paginatedLinks.map((link) => (
                <tr key={link.id} className="hover:bg-gray-50/50 transition-colors group">
                  <td className="px-6 py-5">
                    ...
                    <p className="text-xs text-[#505F76]">General Traffic</p>
                  </td>
                  <td className="px-6 py-5">
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={6} className="px-6 py-12 text-center text-[#505F76] text-sm">No campaigns found. Create your first link above!</td></tr>
              )}
            </tbody>
          </table>
          ...
          {filtered.length > 0 && (
            <div className="px-6 py-4 border-t border-gray-50 flex items-center justify-between text-sm text-[#505F76]">
              ...
              <div className="flex items-center gap-2">
                <button disabled={currentPage === 1} onClick={prevPage} className="px-3 py-1.5 border border-gray-200 rounded-lg text-[#505F76] hover:bg-gray-50 text-xs font-medium disabled:opacity-50 disabled:cursor-not-allowed">Previous</button>
                <span className="text-[#191C1E] font-medium px-2">{currentPage} / {totalPages}</span>
                <button disabled={currentPage === totalPages} onClick={nextPage} className="px-3 py-1.5 border border-gray-200 rounded-lg text-[#505F76] hover:bg-gray-50 text-xs font-medium disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="bg-[#F2F4F6] rounded-2xl border border-gray-100 px-6 py-12  ">
            <div className="flex items-center  gap-3 mb-3">
              ...
              <h3 className="font-bold text-[16px] text-[#191C1E]">Performance Tip</h3>
            </div>
            <p className="text-sm  text-[#505F76] leading-relaxed">
              Your <span className="font-semibold text-[#191C1E]">Summer Campaign</span> link has a conversion rate of 6.9%. Consider sharing this specific link on high-traffic social platforms to maximize your earnings this week.
            </p>
          </div>
          ...
          <div className="bg-[#050C9C]  flex flex-col items-start  justify-center  rounded-2xl px-6 py-12 text-white relative overflow-hidden">
            ...
            <h3 className="text-[20px] font-bold text-white   mb-1">Scale Your Earnings</h3>
            <p className=" text-[14px] text-[#3572EF] mb-4">Join the Premium Affiliate tier for 2x rewards.</p>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-md p-8 shadow-2xl">
            <h2 className="text-xl font-bold text-[#191C1E] mb-4">Create New Link</h2>
            <input
              type="text"
            ...
            <div className="flex gap-3 justify-end">
              <button onClick={() => setShowCreate(false)} className="px-5 py-2.5 text-[#505F76] hover:text-gray-700 text-sm font-medium">Cancel</button>
